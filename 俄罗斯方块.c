...
void DeawMap();									//绘制地图 
Tetris * BlockRand(int code);					//随机方块生成(先生成主方块，再补齐其他方块)  
void JudgeDirection(Tetris ** Block);			//按键响应 
void Form(Tetris ** Block);						//形态变换
void ShowBlock(Tetris ** Block);				//显示完整方块 
int JudgeWall(Tetris ** Block);					//判断左右界限，只需要最左边和最右边即可,以及接触 
void MoveCursor(int x, int y);					//移动光标  
void SetColour(int c);							//颜色设定
int JudgeGroud(Tetris * Phead, Tetris ** Block);//判断落地，同样这里需要检测19种形态 若落地经行下一个方块的刷新 
void JudgeEntire(Tetris * Head);				//判断整行是否填满，若填满则清除该行  
void NewEntire(Tetris * head, int y);			//若上面函数执行清行，则刷新该行上面地图  
void Show(int n);								//显示附加信息
void Location(Tetris ** Block, int x, int y, int a, int b, int c, int d, int e, int f); //确定其他三个方块位置 
void Location_y(Tetris ** Block, int x, int y, int a, int b, int c, int d, int e, int f); //坐标更新
